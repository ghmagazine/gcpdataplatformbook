...
def run():
    ...
    event_file_path = 'gs://{}-gcpbook-ch4/data/events/{}/*.json.gz'.format(
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        ...
        users = (
            p
            # BigQueryのユーザ情報を保管するテーブルgcpbook_ch4.usersからデータを
            # 読み取る。
            | 'Read Users' >> beam.io.Read(
                beam.io.BigQuerySource('gcpbook_ch4.users'))
            # 後続の結合処理で必要となるため、キー・バリュー形式にデータを変換する。
        ...
        (
            ...
            # BigQueryのテーブルgcpbook_ch4.dauへ算出結果を書き込む。
